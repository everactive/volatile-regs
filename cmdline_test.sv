...
class cmdline_test extends uvm_test;
   ...
   virtual task run_phase(uvm_phase phase); uvm_coreservice_t cs_ = uvm_coreservice_t::get();
      ...
      $cast(env, uvm_top.find("env"));
	   ...
	   // Exclude `TIMER` from bit-bash test.
	   // Test insists that it reads back what it wrote.
	   // `TIMER` is too volatile for that.
	  uvm_resource_db#(bit)::set({"REG::",env.model.TIMER.get_full_name()},
      ...
      begin : enable_checks_on_all_register_fields
	      uvm_reg_field fields[$];
	      env.model.get_fields(fields, UVM_HIER);
	      foreach (fields[i]) fields[i].set_compare(UVM_CHECK);
      end
